...
export const fetchCompanies = createAsyncThunk('companiesReducer/fetchCompanies', async () => {
  const data = await api.getCompanies().then(res => res && res.json())
  if (!data) {
...
export const fetchFlatsByCompanyId = createAsyncThunk('companiesReducer/fetchFlatsByCompanyId', async id => {
  const data = await api.getFlatsByCompanyId(id).then(res => res && res.json())
  if (!data) {
    throw new Error(data.message || 'Something went wrong!')
  }
...
export const createClient = createAsyncThunk(
  ...
  async ({ name, phone, email }, { dispatch, getState }) => {
    ...
    const data = await api.postClientForm(form).then(res => res && res.json())
...
export const deleteClient = createAsyncThunk(
  ...
  async (id, { dispatch, getState }) => {
    const data = await api.deleteClient(id).then(res => res && res.json())
    if (!data) {
      throw new Error(data.message || 'Something went wrong!')
    }
    dispatch(fetchFlatsByCompanyId(getState().companies.currentCompanyId))
