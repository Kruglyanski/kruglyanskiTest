...
export const selectStreetsArray = createSelector(selectAllFlats, flats =>
  Array.from(new Set(flats.map(i => i.streetName)))
)

export const selectFlatsByStreet = createSelector([selectAllFlats, selectCurrentStreet], (flats, street) =>
  flats.filter(flat => flat.streetName === street)
)
export const selectFlatById = createSelector([selectAllFlats, selectCurrentFlatId], (flats, id) =>
  flats.find(flat => flat.addressId === Number(id))
)

export const selectFlatsByBuilding = createSelector(
  [selectAllFlats, selectCurrentBuilding, selectCurrentStreet],
  (flats, building, street) => flats.filter(flat => flat.building === building && flat.streetName === street)
)

export const selectBuildingsArray = createSelector(selectFlatsByStreet, flats =>
  Array.from(new Set(flats.map(i => i.building)))
)
